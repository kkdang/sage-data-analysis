...
1-Make list of functional categories
```{r}
# This list is obtained from biomart, by asking for all ENSG that have a GO ID.
ENSG_GO = read.delim("../../ENSG_GO_mart_export.txt", header = TRUE)
all_GO = unique(ENSG_GO$GO.Term.Accession)

```

2-Run lm on each category
```{r cache=TRUE}
...
plot(goBetas_reduced[,2],goBetas_reduced[,1], main = "Slope vs Intercept", xlab = "Intercept", ylab = "Slope")
...
4-Choose some categories that have highest rate of detection increase with depth, but take into account the number of genes affiliated with each GO group. Essentially, I just drew a line on the graph and look at things that are above the line.
```{r}
