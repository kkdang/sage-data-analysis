...
xStatus = as.numeric(rownames(testResults$fit) %in% xSig)
limma::plotMA(testResults$fit,status = xStatus,cex=c(0.3,0.5),legend = FALSE,col=c("green3"),main = modelName, ylim = range(-5,5))
...
# Entire suture-type sig list set
cEntrez = univEnrez$entrez[match(DEgeneLists$`Coronal-control sig factors`, univEnrez$`rownames(pc_palx20.dge)`)]
Cpath = enrichPathway(gene=na.omit(cEntrez),universe = as.character(univEnrez$entrez), readable=T)
head(summary(Cpath))
barplot(Cpath,showCategory = 25, main = "Coronal")
enrichMap(Cpath, vertex.label.cex = 0.8)


sEntrez = na.omit(as.character(univEnrez$entrez[match(DEgeneLists$`Sagittal-control sig factors`, univEnrez$`rownames(pc_palx20.dge)`)]))
sPath = enrichPathway(gene=na.omit(sEntrez),universe = as.character(univEnrez$entrez), readable=T)
head(summary(sPath))


mEntrez = na.omit(as.character(univEnrez$entrez[match(DEgeneLists$`Metopic-control sig factors`, univEnrez$`rownames(pc_palx20.dge)`)]))
mPath = enrichPathway(gene=na.omit(mEntrez),universe = as.character(univEnrez$entrez), readable=T)
head(summary(mPath))
```
...
# Variations on a theme of four (three) groups
```{r} 
accumulatedCounts = matrix(ncol=2,dimnames=list(row=NA,col=c("model", "num_sig")))
accumulatedResults = matrix(NA,nrow = nrow(pc_palo.dge),ncol = 1,dimnames = list(rownames(pc_palo.dge),c("default")))
DEgeneLists = list()
...
testResults = calculateDE(in.fit=fit,inContrast=my.contrasts[,1],pcutoff=cutoff,plotTitle=modelName,limma = TRUE)
DEgeneTable = topTable(testResults$fit,number = nrow(in.dge))
xSig = rownames(DEgeneTable)[intersect(which(DEgeneTable$adj.P.Val < cutoff),which(DEgeneTable$logFC >= 1))]
xStatus = as.numeric(rownames(testResults$fit) %in% xSig)
limma::plotMA(testResults$fit,status = xStatus,cex=c(0.3,0.5),legend = FALSE,col=c("green3"),main = modelName, ylim = range(-5,5))
accumulatedCounts = rbind(accumulatedCounts, c(modelName, length(xSig)))
DEgeneLists[[1]] = rownames(DEgeneTable)[which(rownames(DEgeneTable) %in% xSig)]
names(DEgeneLists)[[1]] = modelName
DEfits[[1]] = testResults$fit
names(DEfits)[[1]] = modelName
...
limma::plotMA(testResults$fit,status = xStatus,cex=c(0.3,0.5),legend = FALSE,col=c("green3"),main = modelName, ylim = range(-5,5))
accumulatedCounts = rbind(accumulatedCounts, c(modelName, length(xSig)))
DEgeneLists[[2]] = rownames(DEgeneTable)[which(rownames(DEgeneTable) %in% xSig)]
names(DEgeneLists)[[2]] = modelName
DEfits[[2]] = testResults$fit
names(DEfits)[[2]] = modelName
...
limma::plotMA(testResults$fit,status = xStatus,cex=c(0.3,0.5),legend = FALSE,col=c("green3"),main = modelName, ylim = range(-5,5))
accumulatedCounts = rbind(accumulatedCounts, c(modelName, length(xSig)))
DEgeneLists[[3]] = rownames(DEgeneTable)[which(rownames(DEgeneTable) %in% xSig)]
names(DEgeneLists)[[3]] = modelName
DEfits[[3]] = testResults$fit
names(DEfits)[[3]] = modelName
...
limma::plotMA(testResults$fit,status = xStatus,cex=c(0.3,0.5),legend = FALSE,col=c("green3"),main = modelName, ylim = range(-5,5))
accumulatedCounts = rbind(accumulatedCounts, c(modelName, length(xSig)))
DEgeneLists[[4]] = rownames(DEgeneTable)[which(rownames(DEgeneTable) %in% xSig)]
names(DEgeneLists)[[4]] = modelName
DEfits[[4]] = testResults$fit
names(DEfits)[[4]] = modelName
par(op)
...
```{r echo=FALSE}
accumulatedCounts = accumulatedCounts[-1,]
venn.diagram(list(Coronal=DEgeneLists$`Coronal-control sig factors`,Sagittal=DEgeneLists$`Sagittal-control sig factors`,Metopic=DEgeneLists$`Metopic-control sig factors`,Lambdoid=DEgeneLists$`Lambdoid-control sig factors`),filename="paper1/figure\ drafts/venn_case-control_4groups_FCgt1.tiff",fill = c("grey63","gold2","coral","white"))

venn.diagram(list(Coronal=DEgeneLists$`Coronal-control sig factors`,Sagittal=DEgeneLists$`Sagittal-control sig factors`,Metopic=DEgeneLists$`Metopic-control sig factors`),filename="paper1/figure\ drafts/venn_case-control_3groups_FCgt1.tiff",fill = c("grey63","gold2","coral"))
...
sPath = enrichPathway(gene=sEntrez,universe = as.character(univEnrez$entrez), readable=T)
head(summary(sPath))
barplot(sPath,showCategory = 25, main = "Sagittal")
enrichMap(sPath, vertex.label.cex = 0.8)


mEntrez = na.omit(as.character(univEnrez$entrez[match(DEgeneLists$`Metopic-control sig factors`, univEnrez$`rownames(pc_palx20.dge)`)]))
mPath = enrichPathway(gene=mEntrez,universe = as.character(univEnrez$entrez), readable=T)
head(summary(mPath))
barplot(mPath,showCategory = 25, main = "Metopic")
enrichMap(mPath, vertex.label.cex = 0.8)
