...
generateDataAndClinicalObj=function(synId){
  ...
  casesStatus[which(colnames(estReads) %in% paste("X", controlSampleIDs, sep=""))] = "control"
...
generate_PCA_datasets=function(in.dge,pcNames){
...
pc_palo.prc = generate_PCA_datasets(in.dge = data.dge, pcNames=proteinCoding_transcripts)
...
head(clinicalDataR)
clinicalDataR = clinicalDataR[match(colnames(data.dge),clinicalDataR$"Px Code"),]
pc_palo.prc = generate_PCA_datasets(in.dge = data.dge, pcNames=proteinCoding_genes)
calc_plot_PC_corr(in.prc = pc_palo.prc,inClinical = clinicalDataR)
calc_plot_alignPC_corr(in.prc = pc_palo.prc,inAlign = alignToCheck)
