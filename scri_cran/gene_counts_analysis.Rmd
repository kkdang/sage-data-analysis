...
```{r cache=TRUE}
metricsEntity = synGet('syn2821246')
counting = read.delim(getFileLocation(metricsEntity), header = TRUE, row.names = 1, sep = " ")
countingDM = data.matrix(counting)
hist(countingDM[1,], main = "distribution across samples of total counts")
pie(rowMeans(countingDM))
```
...
library('R.utils')
geneCounts  = read.delim(gunzip(getFileLocation(dataEntity)), header=T, row.names = 1, sep = " ")
...
casesStatus[which(colnames(geneCounts) %in% paste("X", controls, sep=""))] = "control"
...
Functional Profiles using GO of detected genes


GO "Cellular Component" ontology, level 6
```{r cache=TRUE}
#data.cc6.profiles = basicProfile(genelist=detect_entrez$entrez,onto="CC",level=6,orgPackage="org.Hs.eg.db", empty.cats=FALSE)
#save(data.cc6.profiles,file="data_cc6_profiles.bz2",compress="bzip2")
load("data_cc6_profiles.bz2")
#print(data.cc6.profiles, percentage=TRUE)
#hist(data.cc6.profiles[[1]]$Frequency, breaks = 20)
z = which(data.cc6.profiles[[1]]$Frequency < 300)
data.cc6.profiles.reduced = data.cc6.profiles
data.cc6.profiles.reduced[[1]] = data.cc6.profiles.reduced[[1]][-z,]
rm(z)
plotProfiles(data.cc6.profiles.reduced, "Major functional profiles\nall samples",percentage = F, multiplePlots = F,labelWidth=28)
```

GO "Biological Process" ontology, level 4
```{r cache=TRUE}
#data.bp4.profiles = basicProfile(genelist=detect_entrez$entrez,onto="BP",level=4,orgPackage="org.Hs.eg.db", empty.cats=FALSE)
#save(data.bp4.profiles,file="data_bp4_profiles.bz2",compress="bzip2")
load("data_bp4_profiles.bz2")
#print(data.bp4.profiles, percentage=TRUE)
#hist(data.bp4.profiles[[1]]$Frequency, breaks = 20)
z = which(data.bp4.profiles[[1]]$Frequency < 1700)
data.bp4.profiles.reduced = data.bp4.profiles
data.bp4.profiles.reduced[[1]] = data.bp4.profiles.reduced[[1]][-z,]
rm(z)
plotProfiles(data.bp4.profiles.reduced, "Major functional profiles\nall samples",percentage = F, multiplePlots = F,labelWidth=27)
```

GO "Molecular Function" ontology, level 5
```{r cache=TRUE}
#data.mf5.profiles = basicProfile(genelist=detect_entrez$entrez,onto="MF",level=5,orgPackage="org.Hs.eg.db", empty.cats=FALSE)
#save(data.mf5.profiles,file="data_mf5_profiles.bz2",compress="bzip2")
load("data_mf5_profiles.bz2")
#print(data.mf5.profiles, percentage=TRUE)
#hist(data.mf5.profiles[[1]]$Frequency, breaks = 50)
z = which(data.mf5.profiles[[1]]$Frequency < 200)
data.mf5.profiles.reduced = data.mf5.profiles
data.mf5.profiles.reduced[[1]] = data.mf5.profiles.reduced[[1]][-z,]
rm(z)
plotProfiles(data.mf5.profiles.reduced, "Major functional profiles\nall samples",percentage = F, multiplePlots = F,labelWidth=28)
```
...
boxplot(cpm(data.dge,normalized.lib.sizes=TRUE,log=TRUE)[26200,], main = "IGF1")
