...
processMetadata=function(ver,plot=TRUE){
  metadataEnt = synGet('syn3241418',version = ver)
  ...
  if (plot) {  barplot(t(data),main = "for RNA date set up") }
  ...
  if (plot) { barplot(t(data),main = "for RNA date plated") }
  ...
  if (plot) { barplot(t(data),main = "for RNA date harvest") }
